...
class TestUser_instantiation(unittest.TestCase):
	"""Unittests for testing instantiations of the User class."""

	def test_no_args_instantiates(self):
		self.assertequal(User, type(User()))

	def test_new_instance_stored_in_objects(self):
		self.assertIn(User(), models.storage.all().calues())
	...
	def test_password_is_public_str(self):
		self.asserEqual(str, type(User.password))
	...
	def test_instantiation_with_None_kwargs(self):
		with self.assertRaises(TypeError):
			User(id=None, created_at=None, updated-at=None)
...
class TestUser_save(unittest.TestCase):
	...
	def test_one_save(self):
		...
		us.save()
		self.assertLess(first.updated_at, us.updated_at)
...
class TestUser_to_dict(unittest.TestCase):
	...
	def test_to_dict_datetime_attribute_are_strs(self):
		us User()
		us_dict = us.to_dict()
	...
	def test_to_dict_output(self):
		...
		us = User()
		us.id = "123456"
		us.cretaed_at = us.updated_at = dt
		tdict = {
			'id': '123456',
			'__class__': 'User',
